...
class ApplicationsSentScreen extends StatefulWidget {
  const ApplicationsSentScreen({super.key});

  @override
  State<ApplicationsSentScreen> createState() => _ApplicationsSentScreenState();
}

class _ApplicationsSentScreenState extends State<ApplicationsSentScreen> {
  ...
  Future<void> _loadApplications() async {
    ...
    try {
      ...
      setState(() => _error = 'Impossible de charger vos candidatures.');
    } finally {
    ...
    if (_applications.isEmpty) {
      return Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Icons.send_outlined, size: 48, color: AppColors.slate400),
            const SizedBox(height: 16),
    ...
    return RefreshIndicator(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          return _ApplicationCard(app: _applications[index]);
        },
      ),
    );
  }
          ...
          children: [
                  ...
                  ? ClipRRect(
                      ...
                      child: Image.network(logo, fit: BoxFit.cover,
                          errorBuilder: (_, __, ___) =>
                              const Icon(Icons.business, color: AppColors.slate400, size: 24)),
                    )
                  : const Icon(Icons.business, color: AppColors.slate400, size: 24),
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(title, style: AppTextStyles.heading3, maxLines: 1, overflow: TextOverflow.ellipsis),
